...
/**
 * userRewards never decrease over time
 */
rule userRewards_not_decrease_over_time() {
    address[] assets;
    address user;
    address reward;
    env e1;
    ...
    // simplifications to avoid timeout
    require assets.length == 1;
    require getAssetDecimals(assets[0]) == 6;

    uint256 userRewards1 = getUserRewards(e1, assets, user, reward);
    uint256 userRewards2 = getUserRewards(e2, assets, user, reward);
    assert userRewards1 <= userRewards2;
}
